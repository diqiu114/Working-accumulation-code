...
class MyMainWindow(QMainWindow, Ui_Form):
    ...
    def __init__(self, parent=None):
        ...
        self.palette.setColor(QPalette.Base, QColor(0, 0, 255))  # 使用RGB颜色设置QTextEdit的背景颜色为红色
        self.textEdit_rgb_send.setPalette(self.palette)
    ...
    def Com_Receive_Data(self):
        ...
        if self.hexShowing_checkBox.isChecked() == False :
            try:
                rx_text = rxData.decode('UTF-8')
                self.textEdit_Recive.insertPlainText(rx_text)
            except:
                pass
            numbers = [int(n) for n in rx_text.split(",")]
            print(numbers)
            # 使用RGB颜色设置QTextEdit的背景颜色
            self.palette.setColor(QPalette.Base, QColor(numbers[0], numbers[1], numbers[2]))
            # self.palette.setColor(QPalette.Base, QColor(0, 100, 0))

            self.textEdit_Recive.setPalette(self.palette)
